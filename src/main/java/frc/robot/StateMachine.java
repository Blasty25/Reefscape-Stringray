...
public class StateMachine extends SubsystemBase {
  ...
  public void enableStateSetup() {
    ...
    stateTriggers
        ...
        .onTrue(
            Commands.parallel(
                hopper.autoIntake(
                    outtake,
                    StateHandlerConstants.HOPPER_INTAKE_VOLTAGE,
                    StateHandlerConstants.OUTTAKE_INTAKE_VOLTAGE),
    ...
    stateTriggers.get(RobotState.Intake).and(driver.povUp()).whileTrue(hopper.eject());
    // Commands.startEnd(
    // () -> hopper.setTrackPercent(-1.0), () -> hopper.setTrackPercent(0),
    // hopper));
    ...
    stateTriggers
        ...
        .onTrue(autoAlign.visionAutoAlignLeft(drive, elevator)); // , true, elevator));

    stateTriggers
        .get(RobotState.Shoot)
        .and(driver.rightBumper())
        .onTrue(autoAlign.visionAutoAlignRight(drive, elevator)); // false, elevator));
